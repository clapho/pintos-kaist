...
static int64_t global_min_tick = INT64_MAX;
...
void thread_update_min_tick(void)
{
	...
	{
		global_min_tick = INT64_MAX;
	}
	else
	{
		struct thread *t = list_entry(list_front(&sleep_list), struct thread, elem);
		global_min_tick = t->wakeup_tick;
	}
}

int64_t thread_get_min_tick(void)
{
	return global_min_tick;
}
...
bool wakeup_tick_comparator(const struct list_elem *a, const struct list_elem *b, void *aux UNUSED)
{
...
void thread_sleep(int64_t wakeup_time)
{
	...
	{
		global_min_tick = list_entry(list_front(&sleep_list), struct thread, elem)->wakeup_tick;
	}
